...
| `cookie-compliance-digest-2026-09-19.pdf` | Print version of the Cookie Compliance Digest, 74 pages, one numbered source per document. | Same, for cookies. |
...
| `live-recheck-2026-09-20/` | The 149 cookie entries with no downloaded PDF, rechecked against the source page itself: 110 confirmed, 28 tightened, 8 substantive corrections, and **none left unreached**. The corrections include a Korean fine that was out by a factor of one hundred (KRW 6.6 million, not 660 million) and a Saudi direct-marketing carve-out that does not exist. 27 source links were repointed from indexes, trackers and dead pages to the document itself. The report names every host that refused, what refused it and what finally worked, and the three entries that are still short. Contains the report, the full per-entry results, every attempt, the change set and `apply_recheck.py`. | Before re-running a link check or a download pass, and when a cookie source link looks wrong. |
| `law-verification-2026-09-20.md` | Check of the site against the two regulations themselves, not the documents that comment on them: all 133 AI Act Digest Part 1 entries against the consolidated text of Regulation (EU) 2024/1689 as amended by the Digital Omnibus (EUR-Lex CELEX 02024R1689-20260727), and every GDPR article citation in the EDPB and cookie digests against Regulation 2016/679. 85 entries confirmed, 34 tightened, 14 substantive corrections: the application dates of Articles 78 and 101 were wrong, four Omnibus notes described the law backwards, five amended provisions had no note, and two counts had gone stale. Of about 350 GDPR citations one was wrong. The same pass covered the two advice pages, which had never been verified: of 37 citations three were wrong and are fixed in place (Article 48 GDPR stated without its saving clause for other Chapter V grounds, Article 27(5) of the AI Act credited to the Omnibus, and Article 4(1)'s own words attributed to Commission guidance). Raw results, the change set and the agent briefs are in `law-verification-2026-09-20/`. | Before trusting an application date, an Omnibus note or an article citation, and before editing a Part 1 entry. |
| `law-verification-2026-09-20/` | `law_review_changes.json` and `apply_law_review.py` (replays the 66 AI Act field changes against `items_a.json` to `items_e.json`, asserting on the exact text it expects, so a rerun after another edit fails loudly), the full per-entry results for the AI Act and GDPR passes, the recital sweep, and `LAWCHECK.md` and `GDPRCHECK.md`, the briefs the checking agents worked from. | You rebuild the AI Act data files, or want the evidence behind a correction. |
