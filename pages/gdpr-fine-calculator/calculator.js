...
(function () {
		...
		for (var i = 0; i < steps.length; i++) {
			...
			if (comps.length >= MIN_COMPANIES || i === steps.length - 1) {
				return {
					...
					droppedType: !!type && !s.type, droppedSector: !!sector && !s.sector,
	...
	var TIER_BY_TYPE = {
		"any":         { tier: 4, why: "With no violation type chosen the higher tier is assumed, which is both the cautious reading and where 73% of the cases in this data sit. If the decision cited only Art. 28, 31, 32, 33, 34 or 58, the ceiling would be the 2% one instead." },
	...
	function render() {
		...
		var typeChoice = document.getElementById("fc-type").value;
		var type = typeChoice === "any" ? null : typeChoice;
		...
		var tierInfo = TIER_BY_TYPE[typeChoice] || TIER_BY_TYPE["any"];
		...
		h += '<div class="fc-basis"><strong>What this is built on.</strong> '
			...
			+ (peers.usedType ? "for " + typeLabel(peers.usedType).toLowerCase()
	...
	function init() {
		...
		anyOpt.value = "any";
		anyOpt.textContent = "Not sure, or more than one  (" + ROWS.length + " cases with a turnover)";
		...
		var ANY_NOTE = "Leave this as it is if you do not know which Article is in play, or if "
			+ "more than one applies, which is usual. It costs little: once organisations of a "
		...
		typeSel.addEventListener("change", function () {
			...
			document.getElementById("fc-typenote").textContent = t ? t.note : ANY_NOTE;
			render();
		});
